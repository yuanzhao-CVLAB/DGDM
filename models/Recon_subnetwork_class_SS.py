...
class AttentionBlock(nn.Module):
    ...
    def __init__(self, in_channels, n_heads=1, n_head_channels=-1,group_num=32):
        ...
        self.class_linear1 =nn.Sequential( nn.Linear(384,in_channels*2), nn.SiLU(),
            nn.Dropout(p=0),)
    def forward(self, x, time=None,control=None):
        ...
        if class_features is not None:
            class_features = self.class_linear1(class_features).unsqueeze(-1).chunk(2,dim = 1)
...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, inputs):
        # 输入 shape: (N, C, H, W)
        inputs = inputs.permute(0, 2, 3, 1).contiguous()  # 变为 (N, H, W, C)
        input_shape = inputs.shape

        # Flatten 输入数据为 (N*H*W, C)
        flat_input = inputs.view(-1, self.embedding_dim)
        # flat_input = inputs
        # 计算每个输入向量到所有编码向量的欧氏距离
        distances = (torch.sum(flat_input ** 2, dim=1, keepdim=True)
                     + torch.sum(self.embedding.weight ** 2, dim=1)
                     - 2 * torch.matmul(flat_input, self.embedding.weight.t()))

        # 取距离最小的编码向量索引
        min_distances = distances.min(dim=1)[1].unsqueeze(1)

        # 根据索引取出对应编码向量
        quantized = self.embedding(min_distances).squeeze(1)

        # 计算量化误差和承诺损失
        quantization_error = (quantized.detach() - flat_input) ** 2
        commitment_loss = (quantized - flat_input.detach()) ** 2

        loss = quantization_error + self.commitment_cost * commitment_loss

        # 存储损失到属性中
        self.quantization_loss = loss.mean()  # 或者根据需要进行聚合

        # 恢复量化输出的形状
        quantized = quantized.view(input_shape).permute(0, 3, 1, 2)
        return quantized

class VectorQuantizer_MtiCls(nn.Module):
    def __init__(self, embedding_dim,num_embed,num_cls = 15):
        super(VectorQuantizer_MtiCls, self).__init__()
...
class VectorQuantizer_notrain(VectorQuantizerEMA):
    def __init__(self, num_embeddings, embedding_dim):
        super(VectorQuantizer_notrain, self).__init__(num_embeddings, embedding_dim)
        self.embedding_dim = embedding_dim
        self.num_embeddings = num_embeddings
        self.embedding = nn.Embedding(num_embeddings, embedding_dim)
        self.embedding.weight.data.uniform_(-1, 1)
    ...
    def forward(self, inputs):
        ...
        # Flatten input
        inputs = inputs.view(-1, self._embedding_dim)
...
class QKVAttention(nn.Module):
    ...
    def __init__(self, n_heads,in_channels):
        ...
        self.bottleneck1 = LiteBottleneck(in_channels,1)
        self.conv_vq = nn.Conv2d(in_channels , in_channels, 2,stride=2)
        # self.bottleneck =  nn.Sequential(
        #             nn.Conv2d(in_channels, in_channels//8, 1, stride=1),
        #             GroupNorm32(in_channels//8, in_channels//8),
        #             nn.SiLU(),
        #             nn.Conv2d(in_channels//8 , in_channels//8, 3,stride=2, padding=1),
        #             GroupNorm32(in_channels//8, in_channels//8),
        #             nn.SiLU(),
        #
        #         nn.Conv2d(in_channels // 8, in_channels, 1, stride=1),
        #         GroupNorm32(in_channels//8, in_channels),
        #         nn.SiLU(),
        #         )
        self.linear_gate = nn.Linear(in_channels,in_channels)
        self.in_channels = in_channels
        self.vq1 = VectorQuantizer_MtiCls(8,in_channels)
        self.DGP = DGP(in_channels,in_channels)
    def forward(self, qkv, time=None,control=None):
        ...
        if control.get("similarity") is not None and weight.shape[-1] in control["similarity"].keys():
            similarity_map =control["similarity"][weight.shape[-1]].flatten(-2,-1).repeat(self.n_heads,1)
            weight = weight* torch.sqrt(similarity_map.unsqueeze(1)*similarity_map.unsqueeze(-1))
        if control.get("pred_x0") is not None and control["cond"]:
            ###solid###
            # solid_guidence = 1-ssim_conv(control.get("pred_x0"),control.get("augmented_image"))
            # solid_guidence = F.adaptive_avg_pool2d(solid_guidence,int(math.sqrt(length))).flatten(1,-1)
            # solid_guidence = solid_guidence.sigmoid()
            # solid_v_vec =solid_guidence.unsqueeze(-1)
            # solid_h_vec = solid_guidence.unsqueeze(1)
            # #是否
            # weight= self.alpha*(solid_v_vec@solid_h_vec).repeat(self.n_heads,1,1)*weight
            ...
            ####soft####
            x_0 = self.conv1(control.get("augmented_image"))#.flatten(-2,-1)
            # N,C,HW = x_0.shape
            ...
            activate  = self.dis_conv(diff_map)
            activate = self.attention(F.interpolate(activate,size = (HW,HW),mode = "bilinear"))


            prototype = x_0#*activate
            # images = torch.cat([control.get("pred_x0"),control.get("augmented_image")],1)
            # f = self.conv(images)
            prototype = self.bottleneck1(prototype)
            prototype = self.vq1(prototype, cls_idx)
            prototype = self.conv_vq(prototype)
            prototype =F.interpolate(prototype,size = (HW,HW),mode = "bilinear")
            a = self.DGP(a,prototype,activate).flatten(-2,-1)
            # a = a.flatten(-2,-1)
            # f = F.adaptive_avg_pool2d(f,1)
            # a =a*self.linear_gate(f).sigmoid()+f
            # a = a.permute(0,2,1)
            # f = f.flatten(-2,-1).permute(0,2,1)
            # a = self.channel_attn(f,a,a).permute(0,2,1)
        else:
            a = torch.einsum("bts,bcs->bct", weight, v)
...
class ResBlock(TimestepBlock):
    def __init__(
        ...
        #     nn.Sigmoid()
        # )
        self.class_linear1 = nn.Sequential(GroupNorm32(group_num, time_embed_dim),nn.Linear(time_embed_dim,out_channels*2), nn.SiLU(),nn.Dropout(p=dropout),)
    def forward(self, x, time_embed,control):
        ...
        if control.get("features") is not None  and control["cond"]:
            features =  self.class_linear1(control.get("features")).unsqueeze(-1).unsqueeze(-1).chunk(2,dim=1)
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        for i, mult in enumerate(channel_mults):
            ...
            if i != len(channel_mults) - 1:
                ...
                ch = out_channels
                channels.append(ch)
        self.middle_classify = TimestepEmbedSequential(
            ResBlock(
                ch,
                time_embed_dim=time_embed_dim,
                dropout=dropout,
        ...
        self.middle_cls = nn.Linear(ch, control["classes"])
    ...
    def forward(self, x, time,control):
        ...
        features = self.middle_classify(h, time_embed,control).view(h.shape[:2])
        classes = self.middle_cls(features)
